...
# Gera gráficos para a Parte 3 (peças).
...
name_col = "piece_type"  # é o nome da peça agregada

# 1) Histograma do fator_peca
if "fator_peca" in dfp.columns:
    plt.figure(figsize=(10, 4))
    plt.hist(
        dfp["fator_peca"].dropna(),
        bins=30,
        color=VAMMO_BLUE,
        edgecolor="white",
        alpha=0.9,
    ...
    plt.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label="fator = 1,0 (tempo de tabela)",
    )
    plt.title("Distribuição do fator de tempo por peça\n(tempo real / tempo de tabela)")
...
# 2) TOP 10 por impacto de erro
if "impacto_minutos_aprox" in dfp.columns:
    top_imp = dfp.sort_values("impacto_minutos_aprox", ascending=False).head(10)
    labels = top_imp[name_col].astype(str)

    plt.figure(figsize=(10, 4))
    plt.bar(labels, top_imp["impacto_minutos_aprox"], color=VAMMO_MAGENTA)
    plt.xticks(rotation=45, ha="right")
    plt.ylabel("Impacto aprox. (minutos de erro acumulado)")
    plt.title("TOP 10 peças por impacto de erro de estimativa")

    for x, v in zip(labels, top_imp["impacto_minutos_aprox"]):
...
# 3) TOP 10 por tempo estimado total
